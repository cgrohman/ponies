...
def main():
	...
	horses = []
	for date in dates:
		horses.extend(Horse.findHorses(track='DMR',date=date))

	X=[]
	Y=[]
	for h in horses:
		temp_list=[int(h.age), int(h.gate_position), float(h.jockey['weight']), float(h.odds)]
		X.append(temp_list)
	...
	for h in Horse.findHorses(track='DMR',date='20151129'):
		a=clf.predict([[int(h.age), int(h.gate_position)/10, float(h.jockey['weight'])/10, float(h.odds)]])

	print(clf.predict([[2,12/10,122/10,8.5]]))
